...
inner join filmes f
on f.id_filmes = at.id_filmes
inner join atores a
on at.id_autor = a.id_autor;

SELECT * FROM ATUACOES;

select * from atuacoes at
inner join filmes f
on f.id_filmes = at.id_filmes;
...
from filmes f
inner join atuacoes a on f.id_filmes = a.id_filmes
inner join atores at on a.id_autor = at.id_autor
where at.nome_autor like 'Al Pacino';

select at.nome_autor, at.ano_nascimento
from atores at
inner join atuacoes a on at.id_autor = a.id_autor
inner join filmes f on a.id_filmes = f.id_filmes
where f.titulo like 'jogos vorazes';

select distinct at.nome_autor, at.nacionalidade
from atores at
inner join atuacoes a on at.id_autor = a.id_autor
inner join filmes f on a.id_filmes
where at.nacionalidade like 'reino unido'
and f.genero like 'drama';

select at.nome_autor, f.titulo
from atores at
inner join atuacoes a on at.id_autor = a.id_autor
inner join filmes f on a.id_filmes = f.id_filmes
